...
- Very rarely, games will need to use the `steamclient_experimental` feature. Copy these files in and configure `ColdClientLoader.ini`, then start the game using `steamclient_loader.exe` instead of the normal executable. The only games that I've personally seen require this are Back to the Dawn and Dying Light 2. The game may warn you about "Steam not running" as a hint
