...
**December 2025 NOTE**: Anadius is retiring from maintaining these tools, and they've handed over the source code to the cs.rin admins so that it can be passed on to anyone who wishes to take up the mantle next. As far as I know, these tools still work fine for all current releases, but going forward there may be new versions of Origin DRM that aren't compatible, and you'll definitely need to start legitimately opening any newer games in order to obtain their decryption keys (or just look through the game's thread to see if someone has posted it already). If I had to guess, newer Origin DRM may become incompatible by chance, but the tool should be easy enough to update in order to start working again, so as long as someone is around to take the source code and push an update I wouldn't consider this a death knell
