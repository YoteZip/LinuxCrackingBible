...
**Protip**: When sourcing Steam-based games from places you don't trust, you can verify that the files you've ended up with are a clean copy by visiting a game's SteamDB "Depots" section with the [Get Data from Steam/SteamDB](https://github.com/Sak32009/GetDataFromSteam-SteamDB) userscript installed. `(JUNE 2025 NOTE: The original author seems to be very close to stopping support for this userscript (seemingly due to personal reasons, not complexity). It may break in the future. If it's not working, read the latest comments in the "Main Forum -> Releases" thread (thread ID: "71837"))`. To start the process, select the "Get Data from Steam / SteamDB" prompt at the bottom right of the page, and pick the "Depots" tab. This will display a list of "depots" that a game has registered with Steam. Games often conditionally serve different languages or DLCs using these separate depots, combining them together into a single output at download time. Next, click the green "Add" button next to each "depot" that you're expecting to be present in your local game directory. For example, in the image below I've selected the base game and first DLC for Rimworld:
